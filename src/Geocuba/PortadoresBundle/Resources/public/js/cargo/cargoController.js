...
Ext.onReady(function () {
    ...
    var store_cargo = Ext.create('Ext.data.JsonStore', {
        storeId: 'id_store_cargo',
        fields: [
            {name: 'id'},
            {name: 'nombre'}
        ],
        ...
        listeners: {
            beforeload: function (This, operation, eOpts) {
                Ext.getCmp('id_grid_cargo').getSelectionModel().deselectAll();
                operation.setParams({
    ...
    var find_button = Ext.create('Ext.form.field.Text', {
        ...
        listeners: {
            render: function (field) {
                Ext.getCmp('id_grid_cargo').getStore().on({
            ...
            change: function (field, newValue, oldValue, eOpt) {
                ...
                if (Ext.isEmpty(Ext.String.trim(field.getValue()))) {
                    ...
                    if (marked) {
                        Ext.getCmp('id_grid_cargo').getStore().loadPage(1);
                    }
        ...
        triggers: {
            search: {
                ...
                handler: function () {
                    ...
                    if (!Ext.isEmpty(Ext.String.trim(value))) {
                        this.setMarked(true);
                        if (Ext.getCmp('id_grid_cargo').getStore().getCount() > 0)
                            Ext.getCmp('id_grid_cargo').getStore().loadPage(1, {params: {nombre: value}});
                    }
                }
            },
            clear: {
    ...
    var grid_cargo = Ext.create('Ext.grid.Panel', {
        id: 'id_grid_cargo',
        store: store_cargo,
        columns: [
        ...
        bbar: {
            xtype: 'pagingtoolbar',
            pageSize: 25,
            store: Ext.getStore('id_store_cargo'),
            displayInfo: true,
        },
        plugins: 'gridfilters',
        listeners: {
            selectionchange: function (This, selected, e) {
                if(Ext.getCmp('cargo_btn_mod'))
                Ext.getCmp('cargo_btn_mod').setDisabled(selected.length == 0);
                if(Ext.getCmp('cargo_btn_del'))
                Ext.getCmp('cargo_btn_del').setDisabled(selected.length == 0);
            }
        }
    });

    var panel_cargo = Ext.create('Ext.panel.Panel', {
        id: 'id_panel_cargo',
        title: 'Cargos',
        frame: true,
        closable:true,
