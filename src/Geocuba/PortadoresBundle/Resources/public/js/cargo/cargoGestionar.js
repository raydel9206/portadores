...
Ext.onReady(function () {
                ...
                buttons: [
                    {
                        ...
                        handler: function () {
                            ...
                            if (form.isValid()) {
                                window.hide();
                                App.request('POST', App.buildURL('/portadores/cargo/add'), form.getValues(), null, null,
                ...
                buttons: [
                    {
                        ...
                        handler: function () {
                            ...
                            if (form.isValid()) {
                                ...
                                App.request('POST', App.buildURL('/portadores/cargo/mod'), obj, null, null,
                                    function (response) { // success_callback
                                        if (response && response.hasOwnProperty('success') && response.success) { // success_callback but check if exists errors
                                            window.close();
                                            Ext.getCmp('id_grid_cargo').getStore().loadPage(1);
    ...
    var _btn_Del = Ext.create('Ext.button.MyButton', {
        ...
        width: 100,
        handler: function (This, e) {
            selection = Ext.getCmp('id_grid_cargo').getSelection();
            Ext.Msg.show({
                title: selection.length === 1 ? '¿Eliminar Cargo?' : '¿Eliminar Cargos?',
                message: selection.length === 1 ?
                    Ext.String.format('¿Está seguro que desea eliminar el cargo <span class="font-italic font-weight-bold">{0}</span>?', selection[0].get('nombre')) :
                    '¿Está seguro que desea eliminar los cargos?',
                ...
                fn: function (btn) {
                    if (btn === 'yes') {
                        ...
                        App.request('DELETE', App.buildURL('/portadores/cargo/del'), params, null, null, function (response) { // success_callback
                            if (response && response.hasOwnProperty('success') && response.success) { // success_callback but check if exists errors
                                Ext.getCmp('id_grid_cargo').getStore().reload();
                            }
                        });
                    }
                }
