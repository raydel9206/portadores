Ext.onReady(function(){
    ...
    let panetree = Ext.create('Ext.tree.Panel', {
        ...
        listeners: {
            select: function (This, record, tr, rowIndex, e, eOpts) {
                grid_comp_recarga.enable();
                grid_comp_recarga.focus();
                panetree.collapse();
            },
        ...
        listeners: {
            boxready: function () {
                ...
                let assignGridPromise = new Promise((resolve, reject) => {
                    let i = 0;
                    while(!Ext.getCmp('id_grid_comp_recarga') && i < 5){
                        setTimeout(() => { i++; }, 1000);
                    }
                    resolve(Ext.getCmp('id_grid_comp_recarga'));
                });
    ...
    let store = Ext.create('Ext.data.JsonStore', {
        ...
        proxy: {
            type: 'ajax',
            url: App.buildURL('portadores/comp_recarga/load'),
            reader: {
                rootProperty: 'rows'
            }
        },
        ...
        listeners: {
            beforeload: function (This, operation, eOpts) {
                grid_comp_recarga.getSelectionModel().deselectAll();
                operation.setParams({
    ...
    let monedaCombo = Ext.create('Ext.form.ComboBox', {
        ...
        listeners: {
            select: function (This, newValue, oldValue, eOpts) {
                grid_comp_recarga.getStore().load();
            }
        }
    });
    ...
    var _btn_Export = Ext.create('Ext.button.MyButton', {
        ...
        handler: function (This, e) {
            ...
            // obj.accion = true;
            //
            App.request('POST', App.buildURL('/portadores/comp_recarga/export'), obj, null, null,
    ...
    var grid_comp_recarga = Ext.create('Ext.grid.Panel',{
        id: 'id_grid_comp_recarga',
        disabled: true,
        region: 'center',
        store: store,
        features: [{
        ...
        columns: [
            { text: '<strong>Cuenta</strong>',
                dataIndex: 'cuenta',
                flex: 1
            },{ text: '<strong>Criterio 1</strong>',
                dataIndex: 'tarjeta',
                flex: 1
            },{ text: '<strong>Criterio 2</strong>',
                dataIndex: '',
                flex: 1
            },{ text: '<strong>Criterio 3</strong>',
                dataIndex: '',
                flex: 1
    ...
    var _panel_comp_recarga = Ext.create('Ext.panel.Panel',{
        id: 'id_panel_comp_recarga',
        title: 'Comprobante de Operaciones - Recargas',
        frame : true,
        closable:true,
        layout: 'border',
        items:[panetree,grid_comp_recarga]
    });

    App.render(_panel_comp_recarga);
});