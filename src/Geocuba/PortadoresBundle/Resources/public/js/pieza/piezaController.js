...
Ext.onReady(function () {
    ...
    var store_pieza = Ext.create('Ext.data.JsonStore', {
        storeId: 'id_store_pieza',
        fields: [
            {name: 'id'},
            {name: 'nombre'}
        ],
        proxy: {
            type: 'ajax',
            url: App.buildURL('/portadores/pieza/load'),
            reader: {
                rootProperty: 'rows'
            }
        },
    ...
    var find_button = Ext.create('Ext.form.field.Text', {
        ...
        listeners: {
            render: function (field) {
                Ext.getCmp('id_grid_pieza').getStore().on({
        ...
        triggers: {
            search: {
                ...
                handler: function () {
                    ...
                    if (!Ext.isEmpty(Ext.String.trim(value))) {
                        this.setMarked(true);
                        if (Ext.getCmp('id_grid_pieza').getStore().getCount() > 0)
                            Ext.getCmp('id_grid_pieza').getStore().loadPage(1, {params: {nombre: value}});
                    }
                }
            },
            clear: {
    ...
    var grid_pieza = Ext.create('Ext.grid.Panel', {
        id: 'id_grid_pieza',
        store: store_pieza,
        columns: [
        ...
        bbar: {
            xtype: 'pagingtoolbar',
            pageSize: 25,
            store: Ext.getStore('id_store_pieza'),
            displayInfo: true,
        },
        plugins: 'gridfilters',
        listeners: {
            selectionchange: function (This, selected, e) {
                Ext.getCmp('pieza_tbar').items.each(
    ...
    var panel_pieza = Ext.create('Ext.panel.Panel', {
        id: 'id_panel_pieza',
        title: 'Piezas',
        frame: true,
        closable:true,
