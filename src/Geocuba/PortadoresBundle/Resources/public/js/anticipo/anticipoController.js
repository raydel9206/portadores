...
// Create the combo box, attached to the states data store
    ...
    //     proxy: {
    //         type: 'ajax',
    //         url: App.buildURL('/portadores/anticipo/loadAntRest'),
    //         reader: {
    //             rootProperty: 'rows',
    //             sortRoot: 'id'
    //         }
    ...
    //     proxy: {
    //         type: 'ajax',
    //         url: App.buildURL('/portadores/anticipo/loadRecRest'),
    //         reader: {
    //             rootProperty: 'rows',
    //             sortRoot: 'id'
    //         }
    ...
    let grid = Ext.create('Ext.grid.Panel', {
        ...
        plugins: ['gridfilters', {
            ...
            rowBodyTpl: new Ext.XTemplate(
                ...
                '               <tpl>',
                '                   <tr class="text-center">',
                '                       <td>{nombrepersonaid}</td>',
                '                       <td>{trabajo}</td>',
                '                       <td>{transito_plugin}</td>',
                '                       <td>{terceros_plugin}</td>',
                '                       <td>{excepcional_plugin}</td>',
                '                       <td>{motivo}</td>',
                '                    </tr>',
                '                </tpl>',
                '              </tbody>',
                ...
                if (selections.length > 0) {
                    ...
                    grid_liquidaciones.getStore().load({
                        callback: function (r, options, success) {
                            if (success) {
                                if (r.length > 0) {
                                    // Ext.getCmp('liquidacion_btn_mod').enable();
                                } else {
                                    // Ext.getCmp('liquidacion_btn_mod').disable();
                                }
                            }
                        }
                    });
                    ...
                    // if (Ext.getCmp('liquidacion_btn_mod') !== undefined)
    ...
    let panetree = Ext.create('Ext.tree.Panel', {
        ...
        listeners: {
            select: function (This, record, index, eOpts) {
                ...
                //     callback: function (a, opt, succ) {
                //         if (succ) {
                //             store_rec_to_restore.load({
                //                 callback: function (r, options, success) {
                //                     if (success) {
                //                         if (r.length > 0 || a.length > 0) {
                //                             Ext.getCmp('btn_ant_restaurar').enable();
                //                             Ext.getCmp('btn_ant_restaurar').setStyle('borderColor', 'red');
                //                         } else {
                //                             Ext.getCmp('btn_ant_restaurar').disable();
                //                             Ext.getCmp('btn_ant_restaurar').setStyle('borderColor', 'gray');
                //                         }
                //                     }
                //                 }
                //             });
