Ext.onReady(function () {
    ...
    let action_handler = function (action) {
        let url = App.buildURL(`/portadores/anexo3/${action === 'print' ? 'export' : action}`),
            selection = gridEquiposTecnologicos.getSelection()[0];
        ...
        if (action === 'generate') {
            ...
            App.request('GET', url, { extra_data: Ext.encode(extraData), records: Ext.encode(records)}, null, null, response => {
                if (response && response.hasOwnProperty('success') && response.success) {
