...
Ext.onReady(function () {
    var store = Ext.create('Ext.data.JsonStore', {
        ...
        listeners:{
            beforeload:function (This, operation, eOpts) {
                Ext.getCmp('gridFamiliaId').getSelectionModel().deselectAll();
                // operation.setParams({
                //     nombre:textSearch.getValue()
                // });
            }
        }
    });
    var find_button = Ext.create('Ext.form.field.Text', {
        ...
        listeners: {
            render: function (field) {
                Ext.getCmp('gridFamiliaId').getStore().on({
        ...
        triggers: {
            search: {
                ...
                handler: function () {
                    ...
                    if (!Ext.isEmpty(Ext.String.trim(value))) {
                        this.setMarked(true);
                        if (Ext.getCmp('gridFamiliaId').getStore().getCount() > 0)
                            Ext.getCmp('gridFamiliaId').getStore().loadPage(1, {params: {nombre: value}});
                    }
                }
            },
            clear: {
    ...
    var grid = Ext.create('Ext.grid.Panel', {
        ...
        bbar:{
            xtype:'pagingtoolbar',
            pageSize:25,
            store:Ext.getStore('storeFamiliaId'),
            displayInfo:true,
        },
        plugins:'gridfilters',
        listeners:{
            selectionchange:function (This, selected, e) {
                Ext.getCmp('Familia_tbar').items.each(
