...
// });
    ...
    function fn_actualizar_existenciafinal_combustible(_grid, n) {
        ...
        if (n >= numero_semana)
         ...
         _grid.store.data.items[1].data['' + (3 * n + 3)] = round(parseFloat(_grid.store.data.items[1].data['' + (3 * n + 1)]) * 100 / Ext.getCmp('id_grid_registro_combustible').getSelectionModel().getLastSelected().data.vehiculonorma, 2).toFixed(2);
    ...
    function fn_actualizar_totales(_grid) {
        ...
        var j = 0;
        //while( j < numero_semana && _grid.store.data.items[0].data['' + (j*3 + 1)] == 0) j++;
                                    ...
                                    listeners: {
                                        ...
                                        edit: {
                                            fn: function (editor, e, eOpts) {
                                                let _grid = Ext.getCmp('id_grid_editar_analisis'),
    ...
    var _btn_Del = Ext.create('Ext.button.MyButton', {
        ...
        width: 100,
        handler: function (This, e) {
            selection = Ext.getCmp('id_grid_registro_combustible').getSelection();
            Ext.Msg.show({
